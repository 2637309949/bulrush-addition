...
func (ai *API) One(r *gin.RouterGroup, name string, handlers ...gin.HandlerFunc) *Hook {
	...
	handler := func(c *gin.Context) {
		one(name, c, ai.xorm, opts)
	}
	h := createHooks(ai.xorm, handler)
...
func (ai *API) ALL(r *gin.RouterGroup, name string, handlers ...gin.HandlerFunc) *Hook {
	...
	r.GET(routePrefixs.One(name), array.Append(func(c *gin.Context) {
		...
		h1 := createHooks(ai.xorm, handler)
		h1.Pre(h.pre)
		h1.Post(h.post)
		h1.Auth(h.auth)
